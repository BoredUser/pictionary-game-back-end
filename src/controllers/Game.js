...
const { get3Words, wait, getHints, getScore } = require("../utils/helpers");
...
class Game {
	...
	chosenWord(playerID) {
		...
		return new Promise((resolve, reject) => {
			...
			socket.on(events.CHOOSE_WORD, ({ word }) => {
				socket.to(socket.roomID).emit(events.HIDE_WORD, {
					word: splitter
						.splitGraphemes(word)
	...
	async giveTurnTo(players, i) {
		...
		drawer
			.to(roomID)
			.broadcast.emit(events.CHOOSING_WORD, { name: drawer.player.name });
	...
	onMessage(data) {
		...
		if (distance === 0 && currentWord !== "") {
			...
			if (
				games[socket.roomID].drawer !== socket.id &&
				!socket.hasGuessed
			) {
				...
				socket.broadcast.emit(events.CORRECT_GUESS, {
					message: `${socket.player.name} has guessed the word!`,
					id: socket.id,
				});

				games[socket.roomID].totalGuesses++;
				games[socket.roomID]["Players"][data.id].score += getScore(
					startTime,
					roundTime
				);
				games[socket.roomID]["Players"][
					games[socket.roomID].drawer
				].score += BONUS;
